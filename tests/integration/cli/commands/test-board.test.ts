...
// Mock logger module - @swc/jest compiles imports as non-configurable,
...
describe('test-board command', () => {
  ...
  const logSpy = loggerModule.log as jest.Mock;
  const errorSpy = loggerModule.error as jest.Mock;
  ...
  afterEach(() => {
    // logSpy and errorSpy are jest.fn() from module mock, no need to restore
  });
