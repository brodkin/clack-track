...
// Mock HomeAssistantClient - @swc/jest compiles imports as non-configurable,
// preventing jest.spyOn from redefining module exports
jest.mock('../../../../src/api/data-sources/home-assistant.js', () => {
  const MockHomeAssistantClient = jest.fn().mockImplementation(() => ({
  ...
  return {
    __esModule: true,
    HomeAssistantClient: MockHomeAssistantClient,
  };
});
...
describe('content:test command', () => {
  ...
  describe('edge case error handling', () => {
    ...
    it('should handle HA connection failure when --with-frame flag is used', async () => {
      ...
      (HomeAssistantClient as jest.Mock).mockImplementation(() => ({
    ...
    it('should handle AI provider creation failure when --with-frame flag is used', async () => {
      ...
      const aiMod = await import('../../../../src/api/ai/index.js');
      const createAIProviderMock = aiMod.createAIProvider as jest.Mock;
      createAIProviderMock.mockImplementation(() => {
      ...
      // Cleanup - restore the original mock implementation
      createAIProviderMock.mockImplementation(() => {
        throw new Error('Mock: AI provider not available in tests');
      });
    ...
    it('should handle local HA client disconnect error when --with-frame flag is used', async () => {
      ...
      (haModule.HomeAssistantClient as jest.Mock).mockImplementation(
