...
import { formatDayName, formatDateMonth } from '../../utils/timezone.js';
...
export class ColorBarService {
  ...
  private buildPrompt(season: string, holiday?: string): string {
    const now = new Date();
    const dayName = formatDayName(now); // "TUESDAY"
    const dateMonth = formatDateMonth(now); // "NOV 26"
    const year = now.getFullYear();

    // Build formatted date string: "Tuesday, November 26, 2024"
    const month = dateMonth.split(' ')[0]; // "NOV"
    const day = dateMonth.split(' ')[1]; // "26"
    ...
    const dayNameCapitalized = dayName.charAt(0) + dayName.slice(1).toLowerCase();
    const dateStr = `${dayNameCapitalized}, ${fullMonth} ${day}, ${year}`;
